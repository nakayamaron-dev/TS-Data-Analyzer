...
export class DataDescriptionComponent implements OnInit {
  @ViewChildren(SortableHeader) headers!: QueryList<
    SortableHeader<VariableListTable>
  >;
  ...
  set sortColumn(sortColumn: SortColumn<VariableListTable>) {
  ...
  get contents() {
    return this._contentsList.asObservable();
  }
  ...
  save() {
    this.mongo.updateTagInfo(this.tagInfo!).subscribe(
      (res) => {
        this.modal
  ...
  public sort(
    values: VariableListTable[],
    column: SortColumn<VariableListTable>,
    direction: string
  ): VariableListTable[] {
    if (direction === '' || column === '') {
      return values;
      ...
      return [...values].sort((a, b) => {
        const res = ((v1: string, v2: string) =>
          v1 < v2 ? -1 : v1 > v2 ? 1 : 0)(`${a[column]}`, `${b[column]}`);
