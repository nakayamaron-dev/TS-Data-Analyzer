...
export class ScatterComponent implements OnInit {
  ...
  async setBaseData() {
    ...
    this.xrange =
      this.plotInfo[0].dateRange?.length === 2
  ...
  updateSingleGraph(graphInfo: IplotScatter) {
    ...
    this.influx.getHistoricalData(tagList, From, To).subscribe((res) => {
      const x = res[graphInfo.tag_x]
        ? res[graphInfo.tag_x].map((itm) => itm.value)
        : [];
      const y = res[graphInfo.tag_y]
        ? res[graphInfo.tag_y].map((itm) => itm.value)
        : [];
  ...
  setLayout(graphInfo: IplotScatter, xtitle?: string, ytitle?: string) {
    ...
    (layout.xaxis?.title as Partial<Plotly.DataTitle>).text = xtitle
      ? xtitle
      : graphInfo.tag_x;
    (layout.yaxis?.title as Partial<Plotly.DataTitle>).text = ytitle
      ? ytitle
      : graphInfo.tag_y;
  ...
  changeViewMode(viewTag: boolean) {
    this.plotInfo.forEach((pInfo) => {
      const xtitle = viewTag
        ? pInfo.tag_x
        : this.tagInfo?.items.find((info) => info.tag === pInfo.tag_x)
            ?.description;
      const ytitle = viewTag
        ? pInfo.tag_y
        : this.tagInfo?.items.find((info) => info.tag === pInfo.tag_y)
            ?.description;
  ...
  plotSettingModal(idx: number) {
    this.modal
      .scatterSettingModal(this.plotInfo[idx], this.tagList, this.yrangeList)
      .then(
        (res) => {
          this.isUnSaved = true;
