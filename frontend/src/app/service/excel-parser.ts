...
class XlsxUtility {
    ...
    static cellValueToString(cell: XLSX.CellObject | undefined): string | undefined {
        if ( !cell ) { return undefined; }
    ...
    static cellValueToNumber(cell: XLSX.CellObject | undefined): number | undefined {
        if ( !cell ) { return undefined; }
...
class ParsedTimeseriesValueArray {
    ...
    get array() { return this.valueArray; }
    ...
    valuesWithTag(tag: string): { [timestamp: string]: number } {
        return Object.keys(this.valueArray).reduce<{ [timestamp: string]: number }>(
            (res, timestamp) => {
                ...
                const tagdata = Object.keys(data).filter( key => key === tag).map( itm => data[itm] );
                if ( tagdata.length === 1 ) { res[timestamp] = tagdata[0]; }
                return res;
            },
            {}
        );
...
class ExcelSheetParser {
    ...
    constructor(ws: XLSX.WorkSheet) {
        ...
        if ( this.tags.length > 0 && this.timeStampArray.length > 0 ) { this.isAvailable = true; }
    }

    get tags(): string[] { return Object.keys(this.tagCells); }
    get timeStampArray(): string[] { return Object.keys(this.timeStampCells); }
    ...
    private searchTimeStampRange(): { [timestamp: string]: XLSX.CellAddress } {
        ...
        for ( let c = this.range.s.c; c <= this.range.e.c; c++ ) {
            ...
            if ( Object.keys(timeStrArray).length > 0 ) { break; }
        }
        return timeStrArray;
    }
    ...
    private searchTagRange(): { [tag: string]: XLSX.CellAddress } {
        ...
        for ( let r = this.range.s.r; r <= this.range.e.r; r++ ) {
            ...
            if ( Object.keys(tagArray).length > 0 ) { break; }
        }
        return tagArray;
    }
    ...
    parseExcelFile(): ParsedTimeseriesValueArray | undefined {
        if ( this.isAvailable === false ) { return undefined; }

        const array =  this.timeStampArray
            .reduce<{ [timeStamp: string]: { [tag: string]: number }}>( (result, timestamp) => {
            result[timestamp] = this.tags.reduce<{ [tag: string]: number }>( (res, tag) => {
        ...
        return Object.keys(array).length > 0 ? new ParsedTimeseriesValueArray(array) : undefined;
    }
}
...
export class ExcelParser {
    ...
    constructor(wb: XLSX.WorkBook) {
        this.workbook = wb;
        this.sheets = wb.SheetNames.map( name => new ExcelSheetParser(wb.Sheets[name]));
    }
