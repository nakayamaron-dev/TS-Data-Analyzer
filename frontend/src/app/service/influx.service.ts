...
export class InfluxService {
  ...
  getHistoricalData(
    ...
    return this.http
      .get<IGetData[]>(`/api/v1/ts/${measurement}/` + qstr, {
        responseType: 'json',
      })
      .pipe(
        map((res) => {
  ...
  getTagList(measurement: string = 'rawdata'): Observable<string[]> {
    return this.http
      .get<IGetData[]>(`/api/v1/ts/${measurement}/last`, {
        responseType: 'json',
      })
      .pipe(
        map((res) => {
  ...
  writeData(lineProtocolData: string[]): Observable<HttpResponse<any>> {
    return this.http.post(
      '/api/v1/ts/rawdata/upload',
      lineProtocolData.join('\n'),
      { observe: 'response' }
    );
  }
  ...
  getDatasetSummary(): Observable<DatasetValuesSummaryInfo> {
    ...
    return this.getTagList().pipe(
      concatMap((tagList) =>
        this.getHistoricalData(tagList).pipe(
          map((res) => {
            (
              Object.keys(
                datasetValuesSummaryInfo
              ) as (keyof DatasetValuesSummaryInfo)[]
            ).forEach((key) => {
              Object.keys(res).forEach((tag) => {
                datasetValuesSummaryInfo[key][tag] = this.my_math.calcSummary(
                  res[tag].map((itm) => itm.value),
                  key
                );
              });
            });
